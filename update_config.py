...
def main():
    ...
    for url in URLS_TO_FETCH:
        ...
        if decoded_content is not None: # 即使是空字典 {} 也添加
    ...
    print(f"\n解码完成，共获得 {len(decoded_data_buffer)} 项内容。准备写入文件...")

    # --- 这里是核心改动 ---
    # 我们不再将所有字典合并成一个。
    # 直接将包含所有解码后内容的列表写入 JSON 文件。
    final_config_list = decoded_data_buffer

    try:
        with open(OUTPUT_FILENAME, 'w', encoding='utf-8') as f:
            # 将整个列表写入文件
            json.dump(final_config_list, f, indent=4, ensure_ascii=False)
        print(f"成功！所有内容已作为一个列表保存到文件: {OUTPUT_FILENAME}")
    except IOError as e:
